...
from recommendation_engine_methods import recommendation_methods
...
def add_variables(df, REF_VAR):
    ...
    colonnes = ['genres', 'actor_1', 'actor_2',
                'actor_3', 'director', 'keywords']
    for categorie in colonnes:
        for index, row in df.iterrows():
            # print(row[categorie],type(row[categorie]))
            try:
                if len(row[categorie]) == 0 or type(row[categorie]) == float:
                    continue
            except:
                continue
            for s in row[categorie]:
                ...
                if type(s) != dict:
                    continue
                if s['name'] in REF_VAR:
                    df.set_value(index, s['name'], 1)
    return df
...
def find_similarities(df, id_entry, del_sequels = True, verbose = False):    
    ...
    for i,s in enumerate(film_selection):
        ...
        if verbose: print("{:<30}".format(s[0]))
